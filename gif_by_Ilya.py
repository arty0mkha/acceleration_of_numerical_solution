...
def anim_result(data_u:np.ndarray, timestep, L, H, picture:bool, showMe:bool):
    ...
    def animate(i):                                                                                     # animate function for animation.FuncAnimation
        ...
        ax.set_title(f'u(x,y,t) at t = {round(i*timestep,4)}')                                               # also change quiver 
        return line
    ...
    ax.set_xlabel('x, m')
    ax.set_ylabel('y, m')
    ax.set_title('u(x,y,t) at t = {t}'.format(t=0))

    ani = animation.FuncAnimation(fig, animate, interval=60, blit=False, frames = data_u.shape[0])       # animate
